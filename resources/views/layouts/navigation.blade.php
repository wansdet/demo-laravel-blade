            ...
            @can('admin', auth()->user())
                ...
                <div class="hidden sm:flex sm:items-center sm:ml-6 flex grow">
                    <x-dropdown align="left" width="48">
                        ...
                        <x-slot name="content">
                        @can('author-blog', auth()->user())
                            <x-dropdown-link :href="route('blog.bloggerIndex')">
                                {{ __('My Blogs') }}
                            </x-dropdown-link>
                        ...
                        @endcan
                        @can('admin-blog', auth()->user())
                            <x-dropdown-link :href="route('blog.adminIndex')">
                                {{ __('Manage Blogs') }}
                            </x-dropdown-link>
                        @endcan
                        <x-dropdown-link :href="route('user.files')">
                            {{ __('My Files') }}
                        </x-dropdown-link>
                        </x-slot>
                    </x-dropdown>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            ...
            <div class="mt-3 space-y-1">
                ...
                @can('admin', auth()->user())
                    @can('author-blog', auth()->user())
                        <x-responsive-nav-link :href="route('blog.bloggerIndex')">
                            {{ __('My Blogs') }}
                        </x-responsive-nav-link>
                    ...
                    @endcan
                    @can('admin-blog', auth()->user())
                        <x-responsive-nav-link :href="route('blog.adminIndex')">
                            {{ __('Manage Blogs') }}
                        </x-responsive-nav-link>
                    @endcan
                    <x-responsive-nav-link :href="route('user.files')">
                        {{ __('My Files') }}
                    </x-responsive-nav-link>
                @endcan
